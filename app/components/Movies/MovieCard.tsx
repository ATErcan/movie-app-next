...
const MovieCard = ({ movie, displayPreview }: { movie: MovieData, displayPreview: (id: string)=>void }) => {
  ...
  return (
    <div id={movie.id.toString()} className="flex flex-col grow-0 shrink-0 basis-auto relative w-48 h-auto cursor-pointer hover:opacity-70 hover:scale-110 md:w-60 xl:w-64" onMouseOver={() => setShowTitle(true)} onMouseOut={() => setShowTitle(false)} onClick={handleClick}>
